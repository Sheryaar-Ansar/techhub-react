...
const Toggle = () => {
  ...
  return (
    <button onClick={()=>dispatch(handleToggle())} className={`absolute top-6 right-6 h-7 w-14 ${!mode ? 'bg-cover bg-center' : 'bg-cover bg-center'} rounded-2xl transition duration-300`} style={!mode ? {backgroundImage: `url(${day})`} : {backgroundImage: `url(${dark})`}}>
